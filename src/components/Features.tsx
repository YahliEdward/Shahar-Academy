...
const FEATURES = [
  ...
  {
    title: 'שריון תור ישיר בוואטסאפ',
    description: 'בוחרים שעה פנויה בלוח, שולחים הודעה אחת — וקיבלתם אישור מיידי. בלי טלפונים, בלי המתנה.',
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" className="w-6 h-6">
  ...
  return (
    <section id="features" className="py-16 px-4 max-w-6xl mx-auto">
      <div className="text-center mb-10">
        <h2 className="text-3xl font-black text-white mb-2">למה ההורים והתלמידים בוחרים בנו</h2>
        <p className="text-slate-400">שלושה דברים שבאמת עושים הבדל בדרך ל־5 יחידות</p>
      </div>
      ...
      <ScrollReveal selector=":scope > div" className="grid md:grid-cols-3 gap-5">
        {FEATURES.map((f, i) => (
          <div
            key={i}
            className="bg-zinc-800/40 border border-zinc-700/50 rounded-2xl p-6 hover:border-yellow-400/30 transition-all"
          >
            <div className="w-12 h-12 rounded-xl bg-yellow-400/15 border border-yellow-400/30 flex items-center justify-center text-yellow-400 mb-4">
              {f.icon}
            </div>
